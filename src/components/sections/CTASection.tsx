  ...
  return (
    <section id="contact" className="relative overflow-hidden ambient-glow" style={{ paddingTop: "var(--fib-8)", paddingBottom: "var(--fib-8)" }}>
      <div className="ds-container">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="max-w-4xl">
          ...
          <TextScramble as="h2" className="font-[family-name:var(--font-display)] font-bold tracking-[-0.03em] text-white" style={{ fontSize: "clamp(var(--text-fib-lg), 5vw, var(--text-fib-xl))", lineHeight: 1.1, marginBottom: "var(--fib-6)" }}>
          ...
          <div className="flex flex-col md:flex-row items-start" style={{ gap: "var(--fib-3)" }}>
            ...
            <a href="/stack" className="inline-block font-[family-name:var(--font-display)] font-bold tracking-[0.15em] uppercase hover:bg-[var(--color-ds-card)] transition-colors duration-200 ds-cta-prismatic" style={{ fontSize: "var(--text-fib-sm)", padding: "var(--fib-3) var(--fib-5)", border: "1px solid var(--color-ds-border-light)", color: "var(--color-ds-text)" }}>
              View Our Stack
            </a>
          </div>
        </motion.div>
