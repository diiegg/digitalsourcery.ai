  ...
  return (
    <section id="process" style={{ paddingTop: "var(--fib-7)", paddingBottom: "var(--fib-7)", backgroundColor: "var(--color-ds-carbon)", borderTop: "1px solid rgba(200, 208, 224, 0.06)", borderBottom: "1px solid rgba(200, 208, 224, 0.06)" }}>
      <div className="ds-container">
        ...
        {/* Flowing SVG connection path — desktop only */}
        <div className="relative">
          <svg className="absolute top-0 left-0 w-full hidden md:block pointer-events-none" style={{ height: "40px" }} viewBox="0 0 1200 40" fill="none" preserveAspectRatio="none">
            <path
              d="M75 20 Q225 5 375 20 Q525 35 675 20 Q825 5 975 20 Q1050 28 1125 20"
              stroke="#E3CCB1"
              strokeWidth="1"
              opacity="0.12"
              fill="none"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4" style={{ gap: "var(--fib-4)", paddingTop: "var(--fib-5)" }}>
            {steps.map((step, i) => {
              const Icon = step.icon;
              const isActive = i === activeStep;

              return (
                <motion.div key={step.num} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] }}>
                  {/* Gradient background card per step */}
                  <div
                    className="relative overflow-hidden"
                    style={{
                      padding: "var(--fib-4)",
                      background: isActive
                        ? "linear-gradient(180deg, rgba(227, 204, 177, 0.04) 0%, transparent 100%)"
                        : "transparent",
                      borderTop: isActive ? "1px solid rgba(227, 204, 177, 0.1)" : "1px solid transparent",
