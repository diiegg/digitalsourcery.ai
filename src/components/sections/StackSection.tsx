...
import { Scan, Server, Wallet, HeartPulse } from "lucide-react";

const verdicts = [
  { icon: Scan, category: "AI Observability", tested: "Datadog vs Grafana+Prometheus vs Dynatrace", verdict: "Grafana + custom ML pipeline: 91% detection, 40% lower cost.", detail: "Datadog catches 83% out of the box. Grafana catches 91% with tuning. We deploy both and let the workload decide." },
  { icon: Server, category: "Model Serving", tested: "vLLM vs TGI vs BentoML vs SageMaker", verdict: "vLLM: 3x throughput. BentoML for multi-model routing.", detail: "SageMaker locks you into AWS pricing. BentoML wins when you need to route between multiple models intelligently." },
  { icon: Wallet, category: "Cost Controls", tested: "Semantic caching, model routing, token budgets", verdict: "Combined approach: 52% total inference cost reduction.", detail: "30-50% query similarity in enterprise workloads. Caching eliminates redundant inference. Model routing drops costs another 20%." },
  { icon: HeartPulse, category: "Self-Healing Infrastructure", tested: "Custom K8s operators vs PagerDuty vs Shoreline", verdict: "Custom operators: 87% auto-healed in <2 minutes.", detail: "Generic AIOps tools don't understand your stack. Custom operators heal OOM kills, disk pressure, and network blips automatically." },
];
            ...
            return (
              <motion.div key={v.category} variants={fadeUp} className="refractive-card cursor-default" style={{ padding: "var(--fib-5)" }}>
                <div className="flex items-start justify-between" style={{ marginBottom: "var(--fib-3)" }}>
                  <h3 className="font-[family-name:var(--font-display)] font-bold text-white" style={{ fontSize: "var(--text-fib-md)" }}>{v.category}</h3>
                  <Icon size={28} style={{ color: "var(--color-ds-crystalline)", opacity: 0.3 }} strokeWidth={1} />
                </div>
